...
const fullYearsOfLife = inventors.reduce(
  (total, currentValue) => total + (currentValue.passed - currentValue.year),
  0
);
console.log(`${fullYearsOfLife} years all the inventors lived all together`);

// 5. Sort the inventors by years lived

console.log("N5");

const sortedByYearsOfLife = [...inventors].sort(
  (a, b) => a.passed - a.year - (b.passed - b.year)
);
console.table(sortedByYearsOfLife);
...
//   const arrayA = lastOne.split(", ");
//   const arrayB = nextOne.split(", ");
...
const sumOfInstances = data.reduce((sum, currentValue) => {
  if (sum[currentValue]) {
    sum[currentValue]++;
  } else {
    sum[currentValue] = 1;
